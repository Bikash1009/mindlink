...
<body>
    ...
    // Format for display (for user)
    $display_date = date('d-m-Y (h:i:sa)');
    echo $display_date;

    // Use MySQL-compatible date for queries
    $today_date = date('Y-m-d');

    $patientrow = $database->query("SELECT * FROM patient;");
    $doctorrow = $database->query("SELECT * FROM doctor;");
    $appointmentrow = $database->query("SELECT * FROM appointment WHERE appodate >= '$today_date';");
    $schedulerow = $database->query("SELECT * FROM schedule WHERE scheduledate = '$today_date';");
?>
    ...
    // Define today and next week in correct format
    $today = date("Y-m-d");
    $nextweek = date("Y-m-d", strtotime("+1 week"));

    // Now $today is defined and ready for use
    $sqlmain = "SELECT 
                    appointment.appoid,
                    schedule.scheduleid,
                    schedule.title,
